...
class ChessLogic:
    ...
    def calculate_all_moves(self) -> list[Move]:
        """Berechnet alle möglichen Züge für alle Figuren auf dem Brett.
        ...
        for piece in self.board.white_pieces:
            get_moves = piece.get_legal_moves(self.board)
    ...
    def all_legal_moves(
        ...
        if not legal_moves:
            return str(self.check_or_stalemate(king, self._all_moves))
    ...
    def _get_king(self, color: str) -> Optional[Piece]:
        ...
        """
        return (
            self.board.white_king if color == 'white'
    ...
    def castle(self, king: Piece) -> list[Move]:
        ...
        if self.is_in_check(king, self._all_moves):
            return []
        
        moves = []
        king_row = king.position[0]
        rook1 = self.board.squares[king_row, 0]  # Queenside
        rook2 = self.board.squares[king_row, 7]  # Kingside
